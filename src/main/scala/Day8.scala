...
object Day8 {
  ...
  sealed trait ConditionOperation
  case object EQ extends ConditionOperation
  case object NE extends ConditionOperation
  case object GT extends ConditionOperation
  case object LT extends ConditionOperation
  case object GTE extends ConditionOperation
  case object LTE extends ConditionOperation

  case class Condition(reg: String, op: ConditionOperation, value: Int)
  case class Instruction(reg: String, op: Operation, value: Int, cond: Condition)

  case class Environment(regs: Map[String, Int], maxValue: Int) {
    def getReg(r: String): Int = regs.getOrElse(r, 0)
    def setReg(r: String, v: Int): Environment = Environment(regs.updated(r, v), math.max(v, maxValue))

    def getMaxReg(): (String, Int) =
      regs.toList.sortBy(_._2).reverse.head
  }

  def mkOperation(input: String): Operation =
    input match {
      case "inc" => Inc
      case "dec" => Dec
    }

  def mkCondOp(input: String): ConditionOperation =
    input match {
      case "==" => EQ
      case "!=" => NE
      case ">"  => GT
  ...
  def parseLine(input: String): Instruction = {
    val fields = input.split("\\s+").map(_.trim)
    Instruction(
      fields(0),
      mkOperation(fields(1)),
      fields(2).toInt,
      Condition(fields(4), mkCondOp(fields(5)), fields(6).toInt))
  }

  def parseInput(input: String): List[Instruction] =
    input.lines.map(parseLine).toList


  def evalCondition(cond: Condition, env: Environment): Boolean =
    cond match {
      case Condition(r, EQ, v)  => env.getReg(r) == v
      case Condition(r, NE, v)  => env.getReg(r) != v
      case Condition(r, GT, v)  => env.getReg(r) > v
      case Condition(r, LT, v)  => env.getReg(r) < v
      case Condition(r, GTE, v) => env.getReg(r) >= v
      case Condition(r, LTE, v) => env.getReg(r) <= v
    }

  def evalInstruction(i: Instruction, env: Environment): Environment =
    i match {
      case Instruction(r, Inc, v, c) if(evalCondition(c, env)) => env.setReg(r, env.getReg(r) + v)
      case Instruction(r, Dec, v, c) if(evalCondition(c, env)) => env.setReg(r, env.getReg(r) - v)
      case _                                                   => env
    }

  def runInstructions(is: List[Instruction]): Environment =
    is.foldLeft(Environment(Map.empty[String, Int], Int.MinValue)) { case (e, i) => evalInstruction(i, e) }

  val smallData = """b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10"""

  val puzzleData = """sdj dec 558 if r <= 8
