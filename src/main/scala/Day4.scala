...
object Day4 {
  ...
  def findValid(input: Iterator[List[String]]) =
    input.filter(isValidPassphrase)

  def countValid(input: String): Int = {
    countValid(input.lines.map(_.split(" ").toList))
  }

  def countValid(input: Iterator[List[String]]): Int = {
    findValid(input).size
  }

  def countValid2(input: String): Int =
    countValid2(input.lines.map(_.split(" ").toList))

  def countValid2(input: Iterator[List[String]]): Int =
    input.filter(isValidPassphrase2).size

  val smallData = """xmuf znkhaes pggrlp zia znkhaes znkhaes
...
"""
  ...
  val puzzleData = """sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
kebx eild nrskdr meja jxczomh gcne""".stripMargin
}
