...
object Day2 {
  def mkMatrix(s: String): List[List[Int]] = s.lines.map(_.split("\\s+").map(_.toInt).toList).toList

  def prob1(input: String): Int = {
    prob1(mkMatrix(input))
  }

  def prob1(matrix: List[List[Int]]): Int = {
    def maxDiff(l: List[Int]): Int = l.max - l.min

    matrix.map(maxDiff).sum
  }

  def prob2(input: String): Int = prob2(mkMatrix(input))

  def prob2(matrix: List[List[Int]]): Int = {
    matrix
      .map { l =>
        l.combinations(2)
        .map { l => l.sorted }
          .collect { case a :: b :: nill if b%a == 0 => b/a }
          .sum  }
      .sum
  }


  val puzzleData = """1640	590	93	958	73	1263	1405	1363	737	712	1501	390	68	1554	959	79
                     ...
                     |1738	207	179	137	226	907	1468	1341	1582	1430	851	213	393	1727	1389	632""".stripMargin
