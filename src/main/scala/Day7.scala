...
object Day7 {
  sealed trait Tree[A] { def value: A; def note: Int }
  case   class LeafNode[A](value: A, note: Int)                      extends Tree[A]
  case   class Node[A](value: A, note: Int, children: List[Tree[A]]) extends Tree[A]
      ...
      env match {
        case Environment(rawData, forest) =>
          rawData.get(name) match {
            case Some(RawData(p, Nil)) => Environment(rawData - name, forest + (name -> LeafNode(p, p.weight)))
            case Some(RawData(p, cs))  =>
              ...
              val newForest  = (env2.forest -- cs) + (name -> Node(p, kidsWeight + p.weight, kids))
  ...
  def findImbalance(tree: Tree[Program], target: Int): (String, Int) = {
    tree match {
      case Node(v, n, cs) if cs.size >= 3 =>
      ...
      case Node(v, n, cs) if cs.size == 2 => ???
      case Node(v, n, cs) if cs.size == 1 => ???
      case LeafNode(v, n)                => ???
    }
  }
...
cntj (57)"""
  ...
  val puzzleData = """qgcmjz (87) -> skzkx, pzkofch
remwlv (18)
